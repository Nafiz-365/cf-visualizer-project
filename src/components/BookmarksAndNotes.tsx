...
import { Bookmark, Edit3, Trash2, Code2, ExternalLink, Search, Tag, X } from 'lucide-react';
...
function BookmarksAndNotesImpl() {
    ...
    const filteredBookmarks = React.useMemo(() => {
        const validActiveTags = Array.from(activeTags).filter(t => allTags.includes(t));

        return bookmarks.filter(b => {
            const note = notes[b.problem_id] || '';
            const searchLower = searchQuery.toLowerCase();
            
            const matchesSearch = b.problem_name.toLowerCase().includes(searchLower) ||
                                  b.problem_id.toLowerCase().includes(searchLower) ||
                                  note.toLowerCase().includes(searchLower);
            
            if (validActiveTags.length === 0) return matchesSearch;
            
            const noteTags = (note.match(/#[\w-]+\b/g) || []).map(t => t.substring(1).toLowerCase());
            const hasAllActiveTags = validActiveTags.every(t => noteTags.includes(t));
    ...
    const renderNoteWithTags = (text: string) => {
        ...
        return parts.map((part, i) => {
            if (part.startsWith('#')) {
                return <span key={i} className="inline-block px-1.5 py-0.5 mx-0.5 rounded text-[10px] font-black bg-brand-secondary/20 text-brand-secondary tracking-widest uppercase">{part}</span>;
            }
        ...
        <Card className="p-6 md:p-8">
            ...
            {filteredBookmarks.length === 0 ? (
                <div className="text-center py-10 text-muted-app">
                    ...
                    <p>{bookmarks.length === 0 ? "No problems saved yet." : "No bookmarks match your search or filters."}</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {filteredBookmarks.map(b => (
                        <div key={b.problem_id} className="p-5 bg-white/5 border border-white/10 rounded-2xl">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-app mt-1">Saved on {new Date(b.created_at).toLocaleDateString()}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <button 
                                        onClick={() => handleRemoveBookmark(b.problem_id)}
                                ...
                                {editingNote === b.problem_id ? (
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            <Button variant="outline" onClick={() => setEditingNote(null)} className="px-4 py-1.5 text-xs rounded-lg">Cancel</Button>
                                            <Button variant="primary" onClick={() => handleSaveNote(b.problem_id)} className="px-4 py-1.5 text-xs rounded-lg bg-brand-secondary border-none text-black font-bold">Save Note</Button>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="group relative">
                                        {notes[b.problem_id] ? (
                                            <p className="text-sm text-muted-app whitespace-pre-wrap leading-relaxed">{renderNoteWithTags(notes[b.problem_id])}</p>
                                        ) : (
                                            <p className="text-sm text-muted-app italic opacity-50">No note added yet. Add #tags to organize!</p>
                                        )}
                                        <button 
                                            onClick={() => {
                                                setEditingNote(b.problem_id);
                                                setNoteDraft(notes[b.problem_id] || '');
                                            }}
