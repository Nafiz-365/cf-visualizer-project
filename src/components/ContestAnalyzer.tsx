...
export function ContestAnalyzer({ ratingHistory, submissions, problemset, userRating, userHandle }: ContestAnalyzerProps) {
    const [selectedContestId, setSelectedContestId] = useState<string>('');

    // Pre-select first contest if available
    React.useEffect(() => {
        if (ratingHistory.length > 0 && !selectedContestId) {
            setSelectedContestId(ratingHistory[ratingHistory.length - 1].contestId.toString());
        }
    ...
    const activeContest = useMemo(() => {
        ...
        return ratingHistory.find((c) => c.contestId.toString() === selectedContestId) || null;
    ...
    const contestSubmissions = useMemo(() => {
        ...
        return submissions
            .filter((s) => s.problem.contestId?.toString() === selectedContestId)
    ...
    const analysis = useMemo(() => {
        ...
        const solvedMap = new Map<string, { time: number; attemptsBefore: number; ok: boolean }>();
        ...
        const paceData = Array.from(solvedMap.entries()).map(([index, info]) => ({
            problem: index,
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-[11px] text-muted-app max-w-sm">
                    This profile has not participated in any rated Codeforces rounds yet. Register and compete to unlock contest insights!
                </p>
            </div>
        );
    }
        ...
        <div className="space-y-5 md:space-y-6 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 md:pb-5 border-b border-white/5">
                ...
                <div className="relative shrink-0 w-full md:w-80">
                    ...
                    >
                        {ratingHistory.slice().reverse().map((c) => (
                            <option key={c.contestId} value={c.contestId} className="bg-bg-app">
                                #{c.contestId} - {c.contestName.substring(0, 36)}...
                            </option>
                        ))}
                    </select>
                            ...
                            <Card className="p-5 md:p-6 bg-linear-to-br from-card-app to-white/1 w-full flex flex-col justify-between h-full">
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-[9px] uppercase font-black text-brand-primary tracking-widest mb-1">
                                            Contest Profile #{activeContest.contestId}
                                        </p>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-3 sm:gap-4 pt-4 border-t border-white/5 space-y-0">
                                        <div className="flex sm:flex-col lg:flex-row justify-between sm:items-start lg:items-center gap-1">
                                            <span className="text-[10px] font-bold text-muted-app uppercase">Your Standing</span>
                                            <span className="text-xs font-mono font-black text-text-app">
                                                Rank #{activeContest.rank}
                                            </span>
                                        </div>
                                        <div className="flex sm:flex-col lg:flex-row justify-between sm:items-start lg:items-center gap-1">
                                            <span className="text-[10px] font-bold text-muted-app uppercase">Rating Delta</span>
                                            <span className={cn(
                                                "text-xs font-mono font-black",
                                                activeContest.newRating - activeContest.oldRating > 0 ? "text-emerald-400" : "text-red-400"
                                            )}>
                                                {activeContest.newRating - activeContest.oldRating > 0 ? '+' : ''}
                                                {activeContest.newRating - activeContest.oldRating}
                                            </span>
                                        </div>
                                        <div className="flex sm:flex-col lg:flex-row justify-between sm:items-start lg:items-center gap-1">
                                            <span className="text-[10px] font-bold text-muted-app uppercase">Performance Shift</span>
                                            <span className="text-[11px] font-mono font-bold text-muted-app">
                                                {activeContest.oldRating} <span className="text-[10px] opacity-40 mx-1">→</span> {activeContest.newRating}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-white/5 space-y-3 mt-4">
                                    <div className="flex items-center gap-3">
                                        <Flame size={14} className="text-brand-secondary animate-pulse" />
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-3 rounded-xl bg-white/3 border border-white/5">
                                            <p className="text-[8px] uppercase text-muted-app font-bold mb-1">Solves</p>
                                            <p className="text-base font-black text-text-app">{analysis?.acceptedCount ?? 0}</p>
                                        </div>
                                        <div className="p-3 rounded-xl bg-white/3 border border-white/5">
                                            <p className="text-[8px] uppercase text-muted-app font-bold mb-1">Total Tries</p>
                                            <p className="text-base font-black text-text-app">{analysis?.totalSubmissions ?? 0}</p>
                                        </div>
                                    </div>
                                </div>
                            </Card>
                    ...
                    <Card className="p-5 md:p-6 space-y-5">
                        ...
                        {contestSubmissions.length > 0 ? (
                                    ...
                                    {analysis?.paceData.map((item, idx) => (
                                        ...
                                        >
                                            ...
                                            <div className="space-y-1.5">
                                                {item.solved ? (
                                                    <div>
                                                        <p className="text-[8px] uppercase text-muted-app font-bold">Solved at</p>
                                                    ...
                                                    <div>
                                                        <p className="text-[8px] uppercase text-muted-app font-bold">Verdict</p>
                                                        <p className="text-sm font-bold text-red-400 font-mono">Unsolved</p>
                                                    </div>
                                                )}
                                                <div className="text-[10px] text-muted-app font-medium">
                                                    Attempts: <strong className="text-text-app">{item.attempts}</strong>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                            ...
                                            >
                                                <div className="flex items-center gap-3 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[9px] font-mono text-muted-app truncate">
                                                            {sub.programmingLanguage} | {sub.timeConsumedMillis}ms
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-1.5 shrink-0">
                                                    <div className={cn(
                                                        ...
                                                        sub.verdict === 'OK' ? "bg-emerald-500" : "bg-red-500"
                                                    )} />
                                                    <span className={cn(
                                                        "text-[9px] font-black uppercase tracking-wider",
                                                        sub.verdict === 'OK' ? "text-emerald-400" : "text-red-400"
                                                    )}>
                                                        {sub.verdict === 'OK' ? 'Accepted' : 'Failed'}
                                                    </span>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            <div className="py-12 text-center opacity-40">
                                <Clock size={28} className="mx-auto mb-2 text-muted-app" />
                    ...
                    <Card className="p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="p-4 rounded-2xl bg-white/2 border border-white/5 space-y-1">
                                <p className="text-[8px] uppercase tracking-wider font-black text-muted-app">Your Standings Rank</p>
                                <p className="text-2xl font-black text-text-app font-mono">#{activeContest.rank}</p>
                                <p className="text-[10px] text-muted-app leading-normal">
                                    Ranked in top {Math.max(1, Math.round((activeContest.rank / 10000) * 100))}% of participants.
                                </p>
                            </div>
                            <div className="p-4 rounded-2xl bg-white/2 border border-white/5 space-y-1">
                                <p className="text-[8px] uppercase tracking-wider font-black text-muted-app">Relative Performance</p>
                                <p className="text-2xl font-black text-brand-primary font-mono">
                                    {activeContest.newRating - activeContest.oldRating > 0 ? 'Optimal' : 'Stable'}
                                </p>
                                <p className="text-[10px] text-muted-app leading-normal">
                                    Your rating changed by {activeContest.newRating - activeContest.oldRating > 0 ? '+' : ''}{activeContest.newRating - activeContest.oldRating} points relative to pre-round level.
                                </p>
                            </div>
                            <div className="p-4 rounded-2xl bg-white/2 border border-white/5 space-y-1">
                                <p className="text-[8px] uppercase tracking-wider font-black text-muted-app">Round Rating Leverage</p>
                                <p className="text-2xl font-black text-brand-secondary font-mono">
                                    {Math.abs(activeContest.newRating - activeContest.oldRating)} RP
                                </p>
                                <p className="text-[10px] text-muted-app leading-normal">
                                    Leveraged maximum delta potential in the competitive pool.
                                </p>
                            </div>
                        </div>
                    </Card>
                        ...
                        {upsolveProblems.length > 0 ? (
                            <div className="space-y-4">
                                <p className="text-[11px] text-muted-app max-w-xl font-medium">
                                    These problems from this round were left unsolved. Capitalize on them now to solidify weak segments and claim your training edge!
                                </p>
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[9px] font-black text-brand-secondary uppercase tracking-widest">
                                                            {p.rating ? `${p.rating} RATED` : 'UNRATED'}
                                                        </span>
                                                        <ArrowUpRight size={12} className="text-muted-app group-hover:text-brand-secondary transition-colors" />
                                                    </div>
                            ...
                            <div className="py-8 text-center opacity-40">
                                ...
                                <p className="text-[9px] text-muted-app mt-1">
                                    You solved every problem in this round. Unbelievable performance!
                                </p>
                            </div>
                        )}
                    </Card>
