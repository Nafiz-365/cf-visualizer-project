...
export default function MagicRings({
    ...
    useEffect(() => {
        ...
        const onMouseMove = (e: MouseEvent) => {
            ...
            // Calculate mouse position relative to the element, even if the event comes from window
        ...
        const animate = (t: number) => {
            ...
            hoverAmountRef.current +=
                ((isHoveredRef.current ? 1 : 0) - hoverAmountRef.current) *
                0.08;
            ...
            uniforms.uScaleRate.value = p.scaleRate ?? 1;
            uniforms.uRingCount.value = p.ringCount ?? 1;
            ...
            uniforms.uRingGap.value = p.ringGap ?? 0;
            uniforms.uFadeIn.value = p.fadeIn ?? 0;
            uniforms.uFadeOut.value = p.fadeOut ?? 0;
            uniforms.uMouse.value.set(
                smoothMouseRef.current[0],
                smoothMouseRef.current[1],
            );
            uniforms.uMouseInfluence.value = p.followMouse
                ? (p.mouseInfluence ?? 0)
                : 0;
